...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, img_size, var=None):
        ...
        if self.training:
            ...
            x2 = xy[..., 0:1] + wh[..., 0:1] / 2
            y2 = xy[..., 1:2] + wh[..., 1:2] / 2
            res = torch.cat((x1, y1, x2, y2, p_conf, p_cls), -1)
            return res.view(bs, -1, 5 + self.nc), p.view(bs, self.na, self.ny, self.nx, self.nc + 5)

# @torch.jit.script
def nms_function(map_iter):
    boxes = map_iter[:, 0:4]
    scores = map_iter[:, 4]
    print("**********************begin*******************************")

    obj_dict = torchvision.ops(boxes, scores, 0.5)
    result = torch.index_select(map_iter, 0, obj_dict)
...
class DarknetInt8(nn.Module):
    def __init__(self, cfg, img_size=(416, 416), arc='default'):
        super(DarknetInt8, self).__init__()
        self.mdef = Darknet(cfg, img_size, arc)
        self.module_defs = self.mdef.module_defs
        self.module_list, self.routs = self.mdef.module_list, self.mdef.routs
        self.yolo_layers = self.mdef.yolo_layers

        self.version = self.mdef.version
        self.seen = self.mdef.seen
...
class Darknet(nn.Module):
    ...
    def forward(self, x, var=None):
        ...
        if self.training:
            ...
            result = torch.cat(io, 1)
            boxes = result[..., 0:4]
            boxes = boxes.unsqueeze(2)
            scores = result[..., 4:5]*result[..., 5:]
